...
| L2 read-only audit — data collection + analysis | worker-delegate (mapper role, hardest tier for complex) |
| L2 read-only audit — final diagnosis/conclusion | Claude direct (never delegate) |
| Complex L1 design/architecture | worker-delegate (hardest tier: gpt-5.5), Claude reviews |
